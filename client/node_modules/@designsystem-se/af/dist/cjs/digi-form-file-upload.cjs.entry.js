...
const FormFileUpload = class {
    constructor(hostRef) {
        ...
        this._uploadButton = undefined;
        this._srTextContainer = undefined;
        this.files = [];
        this.error = false;
        this.errorMessages = [];
        this.fileHover = false;
        this.afLabel = text._t.form.file_upload;
        this.afLabelDescription = undefined;
        this.afVariation = formFileUploadVariation_enum.FormFileUploadVariation.SMALL;
        this.afId = randomIdGenerator_util.randomIdGenerator('digi-form-file-upload');
        ...
        this.afHeadingFiles = text._t.form.file_uploaded_files;
        this.afAnnounceIfOptionalText = undefined;
        this.afFileMaxSizeText = undefined;
        this.afShowThumbnail = false;
        this.afEnablePreview = false;
        this.afValidationText = undefined;
        this.afValidation = formFileUploadVariation_enum.FormFileUploadValidation.DISABLED;
        this.afHeadingLevel = formFileUploadVariation_enum.FormFileUploadHeadingLevel.H2;
    }
    ...
    breakpointHandler(e) {
        if (e.detail.value) {
            if (e.detail.value === utilBreakpointObserverBreakpoints_enum.UtilBreakpointObserverBreakpoints.SMALL) {
                this.afVariation = formFileUploadVariation_enum.FormFileUploadVariation.SMALL;
            }
            else {
    ...
    /**
     * Hämta ut komponentens validitets-state och felmeddelanden
     * @en Get component validity and error messages
     */
    async afMGetValidationState() {
        return { isValid: !this.error, errorMessages: this.errorMessages };
    }
    /**
    ...
    async afMImportFiles(files) {
        ...
        const filesThatArOK = this.errorMessages.length === 0 ? files : this.getFilesThatAreOK(files);
    ...
    addErrorMessage(fileName, message) {
        this.errorMessages.push({
            fileName: fileName,
            message: message
        });
    ...
    validateFileStatus(file) {
        if (file['status'] &&
            file['status'] === formValidationMessageVariation_enum.FormValidationMessageVariation.ERROR) {
            const message = file['error']
                ? file['error']
                : text._t.form.file_error_unknown_error;
    ...
    validateMaxSize(file) {
        if (file.size > this.fileMaxSize) {
            const message = this.afFileMaxSizeText
                ...
                : text._t.form.file_error_too_large(file.name, this.afFileMaxSize);
            this.addErrorMessage(file.name, message);
        }
    }
    validateFileType(file) {
        if (!this.checkAcceptedTypes(file)) {
            this.addErrorMessage(file.name, text._t.form.file_error_filetype_not_allowed(file.name, this.afFileTypes));
        }
    }
    validateFile(file) {
    ...
    onCancelFileHandler(e, id) {
        ...
        fileFromList['error'] = text._t.form.file_upload_canceled;
    ...
    validateIfFileIsAlreadyUploaded(file) {
        for (const item of this.files) {
            if (item.name === file.name) {
                this.addErrorMessage(file.name, text._t.form.file_error_exists(file.name));
            }
        }
    }
    handleFiles(e) {
        var _a, _b;
        this.resetErrors();
        const fileList = e.dataTransfer ? (_a = e.dataTransfer) === null || _a === void 0 ? void 0 : _a.files : (_b = e.target) === null || _b === void 0 ? void 0 : _b.files;
        ...
        if (numberOfFiles > this.afMaxFiles) {
            this.addErrorMessage('', text._t.form.file_error_too_many);
            this.error = true;
            return;
        }
        ...
        const filesThatArOK = this.errorMessages.length === 0
            ? filesToValidate
    ...
    emitFile(file) {
        ...
        if (this.afValidation === formFileUploadVariation_enum.FormFileUploadValidation.ENABLED) {
    ...
    async addFileStatusAndEmitFiles(file) {
        file['id'] = randomIdGenerator_util.randomIdGenerator('file');
        file['status'] = 'pending';
    ...
    handleAllowDrop(e) {
        if (this.afVariation === formFileUploadVariation_enum.FormFileUploadVariation.SMALL)
            return;
        e.stopPropagation();
        e.preventDefault();
    }
    handleOnDragEnterLeave(e) {
    ...
    getFiletypeAndShortFileName(file) {
        let fileType = null;
    ...
    renderErrorMessage() {
        ...
        return (index.h("div", { class: 'digi-form-file-upload__error-list-container ' + showBorder }, index.h("ul", { class: "digi-form-file-upload__error-list", "aria-hidden": "true" }, this.renderErrorMessages()), index.h("button", { type: "button", "aria-label": text._t.form.file_upload_reset_errors_aria_label, class: "digi-form-file-upload__button clear-button hidden-mobile", onClick: () => {
                this.resetErrors(true);
            } }, index.h("span", null, text._t.form.file_upload_reset_errors)), index.h("button", { type: "button", onClick: () => {
                this.resetErrors(true);
            }, class: "digi-form-file-upload__button clear-button hidden-desktop", "aria-label": text._t.form.file_upload_reset_errors_aria_label }, index.h("digi-icon-x-button-outline", { class: "digi-form-file-upload__button-icon clear-icon", "aria-hidden": "true" }))));
    }
    ...
    renderErrorMessages() {
        return this.errorMessages.map((error) => {
            return (index.h("li", null, index.h("digi-form-validation-message", { id: "digi-form-file-upload__error", class: "digi-form-file-upload__error", "af-variation": "error" }, error.message)));
        });
    }
    renderPendingFile(file) {
        return (index.h("div", { class: "digi-form-file-upload__item-pending-container" }, index.h("span", { class: "info-wrapper" }, index.h("digi-icon-spinner", { class: "digi-form-file-upload__spinner" }), index.h("span", { style: { margin: '0' } }, text._t.form.file_upload_progress, " ", index.h("span", { class: "hidden-mobile" }, "|")), index.h("p", { class: "digi-form-file-upload__item-name-pending hidden-mobile" }, this.getFiletypeAndShortFileName(file).name)), index.h("span", { class: "button-wrapper" }, index.h("button", { type: "button", onClick: (e) => this.onCancelFileHandler(e, file['id']), class: "digi-form-file-upload__button digi-form-file-upload__button-abort hidden-mobile", "aria-label": text._t.form.file_cancel_upload_long(file.name) }, text._t.form.file_cancel_upload), index.h("button", { type: "button", onClick: (e) => this.onCancelFileHandler(e, file['id']), class: "digi-form-file-upload__button hidden-desktop", "aria-label": text._t.form.file_cancel_upload_long(file.name) }, index.h("digi-icon-x-button-outline", { class: "digi-form-file-upload__button-icon", "aria-hidden": "true" })))));
    }
    renderImageThumbnail(file) {
        if (this.afShowThumbnail) {
            return (index.h("figure", { class: "digi-form-file-upload__item-text-container-figure", style: { margin: '0' } }, index.h("img", { "aria-hidden": "true", class: "thumbnail", src: `data:${file['type']};base64,${file['base64']}` }), index.h("figcaption", { class: "visually-hidden" }, "Miniatyr av ", file.name)));
        }
        return this.renderCheckmark();
    }
    renderCheckmark() {
        return index.h("span", { class: "digi-form-file-upload__item-green-check" });
    }
    renderPDFThumbnail(file) {
        if (this.afShowThumbnail) {
            return (index.h("span", { class: `${this.afShowThumbnail ? 'digi-form-file-upload__item-pdf-preview-bis' : null}`, "aria-label": file.name }, index.h("svg", { style: { scale: '1' }, width: "23.76", height: "32", xmlns: "http://www.w3.org/2000/svg" }, index.h("path", { d: "M22.888 6.124 17.696.88A2.955 2.955 0 0 0 15.6 0H2.97C1.33.006 0 1.35 0 3.006V29C0 30.657 1.33 32 2.97 32h17.82c1.64 0 2.97-1.343 2.97-3V8.249a3.03 3.03 0 0 0-.872-2.124Zm-2.34 1.88H15.84V3.25l4.709 4.756ZM2.97 29.002V3.006h9.9v6.498c0 .831.662 1.5 1.485 1.5h6.435v17.997H2.97Zm15.481-8.98c-.755-.75-2.908-.544-3.985-.406-1.064-.656-1.775-1.562-2.277-2.893.242-1.006.625-2.537.335-3.5-.26-1.637-2.34-1.474-2.636-.368-.273 1.006-.025 2.406.433 4.193-.619 1.493-1.54 3.499-2.19 4.649-1.238.643-2.909 1.637-3.156 2.887-.204.987 1.609 3.449 4.708-1.95 1.386-.462 2.896-1.031 4.233-1.256 1.17.637 2.537 1.062 3.452 1.062 1.578 0 1.733-1.762 1.083-2.418ZM6.194 24.883c.315-.856 1.516-1.844 1.88-2.187-1.175 1.893-1.88 2.23-1.88 2.187Zm5.049-11.91c.458 0 .414 2.005.111 2.549-.272-.869-.266-2.55-.111-2.55Zm-1.51 8.535c.6-1.056 1.114-2.312 1.528-3.418.514.944 1.17 1.7 1.863 2.219-1.287.268-2.407.818-3.391 1.2Zm8.143-.312s-.31.375-2.308-.488c2.172-.162 2.53.338 2.308.488Z", fill: "#1616B2", "fill-rule": "evenodd" }))));
        }
        return this.renderCheckmark();
    }
    ...
    renderFileName(file) {
        if (this.afEnablePreview) {
            return (index.h("a", { class: "digi-form-file-upload__item-name", href: "#", onClick: (e) => {
                    this.viewUploadedFile(e, file);
                } }, file.name));
        }
        else {
            return index.h("span", { class: "digi-form-file-upload__item-name" }, file.name);
        }
    }
    renderOKFile(file) {
        return (index.h(index.Fragment, null, index.h("span", { class: "digi-form-file-upload__item-text-and-thumbnail-container" }, this.renderThumbnail(file), this.renderFileName(file)), index.h("button", { type: "button", onClick: (e) => this.onRemoveFileHandler(e, file['id']), class: "digi-form-file-upload__button digi-form-file-upload__button--mobile", "aria-label": text._t.form.file_remove(file.name) }, index.h("span", { class: "hidden-mobile" }, text._t.form.file_remove()), index.h("digi-icon-trash", { class: "digi-form-file-upload__button-icon hidden-desktop" }))));
    }
    renderFile(file) {
        switch (file['status']) {
            case 'pending':
    ...
    renderFiles() {
        const filesMap = this.files.map((file, index$1) => {
            {
                return (index.h("li", { class: "digi-form-file-upload__item-container", key: 'file-' + index$1 }, index.h("div", { class: "digi-form-file-upload__item" }, index.h("div", { class: "digi-form-file-upload__item-text-container" }, this.renderFile(file)))));
            }
        });
        return filesMap;
    }
    renderFileContainer() {
        ...
        return (index.h(index.Fragment, null, index.h(this.afHeadingLevel, { class: {
                'digi-form-file-upload__files-heading': true,
                'digi-form-file-upload__files-heading--and-errors': this.error
            } }, this.afHeadingFiles), index.h("ul", { class: "digi-form-file-upload__files-list", tabindex: -1, ref: (el) => {
                this._fileList = el;
            } }, this.renderFiles())));
    }
    render() {
        return (index.h(index.Fragment, { key: '580ccd74dcdc8a15ee7a50105066c6d609e723b7' }, index.h("digi-util-breakpoint-observer", { key: '9b9589b28a18ded2bf67994a439bd536f08584e4', onAfOnChange: (e) => this.breakpointHandler(e) }), index.h("div", { key: '1e5cff6bafb3a81af192e1ee4298bee2240eca33', class: Object.assign({ 'digi-form-file-upload': true }, this.cssModifiers) }, index.h("digi-form-label", { key: '75ae2b56e481e00765e1d43234df14c572ea7967', afFor: this.afId, afLabel: this.afLabel, afDescription: this.afLabelDescription, afRequired: this.afRequired, afAnnounceIfOptional: this.afAnnounceIfOptional, afRequiredText: this.afRequiredText, afAnnounceIfOptionalText: this.afAnnounceIfOptionalText }), index.h("div", { key: 'ed0236252fed830bf472d7921d9dcc5a256a91d8', class: {
            ...
            this.afVariation != formFileUploadVariation_enum.FormFileUploadVariation.SMALL && (index.h("div", { key: 'ddd140e8b75acf45e0a888d7263b0960ff1b7962', class: "digi-form-file-upload__upload-area-overlay" })), index.h("div", { key: '10d4233b2ea238241db72a1707ad42e225642282', class: "digi-form-file-upload__text-area" }, index.h("button", { key: '2a2d261139d36d93e6d0bd7fd137ae7b599cfb19', type: "button", ref: (el) => {
                this._uploadButton = el;
            }, onClick: (e) => this.handleInputClick(e), class: "digi-form-file-upload__button hidden_mobile", style: {
                margin: '0'
            } }, index.h("digi-icon-paperclip", { key: 'df312825690f52fd61b6647abfab6e92f597f598', "aria-hidden": "true", class: "digi-form-file-upload__button-icon paper-clip", slot: "icon" }), this.afUploadBtnText
            ? this.afUploadBtnText
            : text._t.form.file_choose), index.h("input", { key: 'ddf070f55f29363aac37b604324ef79c8012a81f', ref: (el) => {
                this._input = el;
            }, onChange: (e) => this.onButtonUploadFileHandler(e), class: "digi-form-file-upload__input", multiple: true, type: "file", id: this.afId, accept: this.afFileTypes, name: this.afName ? this.afName : null, required: this.afRequired ? this.afRequired : null, onDrop: (e) => this.handleDrop(e), onDragOver: (e) => this.handleAllowDrop(e), "aria-errormessage": "digi-form-file-upload__error", "aria-describedBy": "digi-form-file-upload__error" }), index.h("span", { key: '9979fdc76df838ffc9aa8819bbcbf08c91c049ce' }, this.afVariation !== formFileUploadVariation_enum.FormFileUploadVariation.SMALL &&
            text._t.form.file_dropzone))), index.h("p", { key: '4e77feeda7adc04c8fa4255f3b460703d10bc230', class: "visually-hidden", role: "alert", "aria-atomic": "true", ref: (el) => {
                this._srTextContainer = el;
            } }), (this.files.length > 0 || this.error) && (index.h("div", { key: 'acf58e18643a87eeee54c1f7d6db872fa11598c6', class: "digi-form-file-upload__files" }, this.renderErrorMessage(), this.renderFileContainer())))));
    }
...
FormFileUpload.style = DigiFormFileUploadStyle0;

exports.digi_form_file_upload = FormFileUpload;
