...
export const apiClient = {
  ...
  reviews: {
    ...
    delete: (id: number) => fetchApi(`/reviews/${id}`, {
      method: 'DELETE',
    }),
  },
  ...
  checks: {
    ...
    upsert: (reviewId: string | number, data: {
      requirement: string;
      status?: number;
      comment?: string;
      flag?: number;
    ...
    delete: (checkId: number) => fetchApi(`/reviews/checks/${checkId}`, {
      method: 'DELETE',
    }),
    ...
    bulkPrefill: (reviewId: number, prefills: Array<{
      status: string;
      ids: string[];
      comment: string;
